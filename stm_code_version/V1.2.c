...
double calculate_yaw(double UAV_x, double UAV_y, double UGV_x, double UGV_y)
{
	...
	UGV_x += 300.0;
	UGV_y += 300.0;
    angle_deg = (atan2((UGV_y - UAV_y), (UGV_x - UAV_x))) * (180.0 / M_PI);
...
/*
void quaternion_to_euler(float x, float y, float z, float w, float *roll, float *pitch, float *yaw) {
...
void send_drone_cmd_vel(float linear_xd, float linear_zd, float angular_zd) {
...
void StartDefaultTask(void *argument) {
    ...
    //send_drone_cmd_vel(0.3, 0.1, 0.0);
    ...
    for (;;) {
        ...
        {
        	necc_yaw = calculate_yaw(new_gps_data.latitude, new_gps_data.longitude, gps_data.latitude, gps_data.longitude);
        	if((counter_drone % 2) == 0)
        	{
        		...
        		if(necc_yaw < 0)
        		{
        		    while((yaw < (necc_yaw - add)) || (yaw > (necc_yaw + add)))
        		    {
        		        send_drone_cmd_vel(0.0, 0.0, 0.05);
        		...
        		while(correct_yaw == 1 || correct_yaw == 2 || distance < 5)
        		{
        			send_drone_cmd_vel(0.4, -0.05, 0.0);
        			rcl_publish(&drone_cmd_vel_publisher, &cmd_vel_msg, NULL);
        			rclc_executor_spin_some(&executor, RCL_MS_TO_NS(10));
