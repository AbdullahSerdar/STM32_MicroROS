...
double necc_yaw = 0.0;
double add_cnt_1 = 0.5;
double add_cnt_2 = 0.1;
int correct_yaw = 0;
double distance = 2.0;
double yaw = 0.0;
...
void StartDefaultTask(void *argument) {
    ...
    for (;;) {
        ...
        if (HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_0) == GPIO_PIN_SET)
        {
        	...
        	//add_cnt_1 = (necc_yaw*40)/100;
        	//add_cnt_2 = (necc_yaw*5)/100;
        	...
        	if(necc_yaw < 0)
        	{
        	    while((yaw > (necc_yaw + add_cnt_2)) || (yaw < (necc_yaw - add_cnt_2)))
        	    {
        	    	...
        	    	{
        	    		send_drone_cmd_vel(0.0, 0.0, -0.05, 0.3);
        	...
        	while(distance > 1.0)
        	{
        		if(new_gps_data.altitude < gps_data.altitude + 2.5)
        		{
					send_drone_cmd_vel(0.3, 0.0, 0.3, 0.0);
					rcl_publish(&drone_cmd_vel_publisher, &cmd_vel_msg, NULL);
					rclc_executor_spin_some(&executor, RCL_MS_TO_NS(10));
        		...
        		{
					send_drone_cmd_vel(0.3, 0.0, -0.05, 0.0);
					rcl_publish(&drone_cmd_vel_publisher, &cmd_vel_msg, NULL);
					rclc_executor_spin_some(&executor, RCL_MS_TO_NS(10));
					distance = calculate_distance(new_gps_data.latitude, new_gps_data.longitude, gps_data.latitude, gps_data.longitude);
					correct_yaw = 3;
