...
static void MX_ADC1_Init(void)
{
  ...
  */
  ...
  /** Configure for the selected ADC regular channel its corresponding rank in the sequencer and its sample time.
  */
  /** İkinci ADC Kanalı Yapılandırması */
  sConfig.Channel = ADC_CHANNEL_1;  // Kanal 1'e ayarlandı
  sConfig.Rank = 2;
  if (HAL_ADC_ConfigChannel(&hadc1, &sConfig) != HAL_OK)
  {
    Error_Handler();
...
void StartDefaultTask(void *argument) {
    ...
    rcl_subscription_t new_gps_subscriber; // Yeni GPS aboneliği
    ...
    ret = rcl_subscription_fini(&new_gps_subscriber, &node);  // Yeni GPS aboneliği sonlandırma
